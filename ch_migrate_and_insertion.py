...
def migrate():
    client.execute('CREATE DATABASE IF NOT EXISTS perfdb;')
    ...
    client.execute("""
	  ...
	  `temperament` Enum8('sanguine' = 1, 'choleric' = 2, 'melancholic' = 3, 'phlegmatic' = 4)
          ...
          ORDER BY (birthday_date, children)
          PARTITION BY (religion, current_country)
      """)
...
def insertion():
    ...
    for i in range(200):
        ...
        str1 = f"""
            ...
            """
        if i < 200-1:
            r_str = r_str + str1 + str2
        else:
            r_str = r_str + str1 + ';'
    client.execute(r_str)
